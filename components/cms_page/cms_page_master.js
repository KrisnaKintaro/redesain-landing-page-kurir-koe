    ...
    try {
        switch (targetId) {
            ...
            case 'setting-landing':
                workspace.innerHTML = renderPlaceholder('Pengaturan Global Landing Page');
                break;
            case 'comp-hero':
...
function initCmsPageMasterLogic() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            navLinks.forEach(l => {
                l.className = "cms-nav-link group flex items-center gap-3 text-gray-800 hover:bg-yellow-400/50 px-4 py-3 rounded-xl font-bold text-sm transition-all border border-transparent hover:border-yellow-500/30";
                l.querySelector('i').classList.replace('text-accent', 'text-primary');
            });

            link.className = "cms-nav-link active group flex items-center gap-3 bg-primary text-white px-4 py-3 rounded-xl font-bold text-sm shadow-[0_5px_15px_rgba(0,84,183,0.3)] transition-all hover:-translate-y-0.5";
