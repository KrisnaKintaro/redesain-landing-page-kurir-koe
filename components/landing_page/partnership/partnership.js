async function renderPartnership() {
    ...
    if (optionsContainer && data.options) {
        ...
        data.options.forEach(opt => {
            ...
            // Styling kartu opsi mini
            div.className = "bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg border border-gray-100 transition-all duration-300 group cursor-pointer";
            ...
            div.innerHTML = `
                ...
                <p class="text-gray-600 text-sm mb-6 leading-relaxed">${opt.desc}</p>
                
                <!-- Link Daftar -->
                <a href="${opt.btn_link}" class="inline-flex items-center gap-2 text-primary font-semibold hover:text-accent transition-colors duration-300 group-hover:translate-x-1 transform">
