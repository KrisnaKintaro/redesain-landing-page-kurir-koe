async function renderServices() {
    ...
    if (container && data.items) {
        ...
        data.items.forEach((item, index) => {
            // Wrapper luar KHUSUS buat animasi scroll (jeda & slide-up)
            const wrapper = document.createElement('div');
            wrapper.className = `service-wrapper service-delay-${index + 1}`;
            
            // Card asli kita masukin ke dalemnya (tambahin class h-full biar tingginya rata)
            wrapper.innerHTML = `
                <div class="h-full group bg-white rounded-3xl p-8 shadow-sm hover:shadow-2xl border border-gray-100 hover:border-accent transition-all duration-300 transform hover:-translate-y-2 relative overflow-hidden z-10">
